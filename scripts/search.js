...
document.addEventListener("DOMContentLoaded", () => {
    const recipeSection = document.getElementById("recipes-cards"); // Section où les recettes sont affichées
    const inputMain = document.getElementById("input-main"); // Champ de recherche principal
    const clearButton = document.getElementById("clear-button"); // Bouton pour effacer la recherche
    ...
    const filterRecipes = (query = "") => {
        query = query.trim().toLowerCase(); // Normalisation de la requête en minuscules
        ...
        return recipes.filter((recipe) => {
            ...
            const matchesQuery = !query ||
                ...
                recipe.ingredients.some((ingredient) =>
                    regexQuery.test(ingredient.ingredient.toLowerCase())
                );
            ...
            const matchesIngredients = selectedIngredients.length === 0 ||
                selectedIngredients.every((ingredient) =>
                    recipe.ingredients.some(
                        (recipeIngredient) =>
                        new RegExp(ingredient.toLowerCase(), "i").test(recipeIngredient.ingredient.toLowerCase())
                    )
                );
            ...
            const matchesUtensils = selectedUtensils.length === 0 ||
                selectedUtensils.every((utensil) =>
                    recipe.ustensils.some(
                        (recipeUtensil) =>
                        new RegExp(utensil.toLowerCase(), "i").test(recipeUtensil.toLowerCase())
                    )
                );

            const matchesAppliances = selectedAppliances.length === 0 ||
                selectedAppliances.every(
                    (appliance) =>
                    recipe.appliance &&
                    new RegExp(appliance.toLowerCase(), "i").test(recipe.appliance.toLowerCase())
                );

            // Retourne les recettes qui correspondent à la requête et aux tags sélectionnés
            return matchesQuery && matchesIngredients && matchesUtensils && matchesAppliances;
        });
    };
    ...
    const addTag = (type, tag) => {
        ...
        performSearch(); // Re-filtrer les recettes après l'ajout du tag
    };
    ...
    // Ajouter des écouteurs sur la touche "Entrée" et le bouton de recherche
